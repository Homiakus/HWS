...
1. `IPC_CONTRACT.md` — versioned локальный API UI ↔ `hwsd`.
2. `GNOME_ADAPTER.md` — capability matrix и границы desktop integration.
3. `STORAGE.md` — durable state, snapshots, migrations, corruption/recovery.
4. `OBSERVABILITY.md` — correlation IDs, logs, traces, diagnostics bundle.
5. `PERFORMANCE.md` — budgets, benchmarks и profiling protocol.
6. `PACKAGING.md` — install/update/disable/uninstall/fallback.
7. `CONFIG_SCHEMA.md` — пользовательская иерархия и workspace definitions.
8. `PROVIDER_MODEL.md` — dynamic providers, permissions и lifecycle.
